...
const allIcons = [
  'mail', 'close', 'download', 'code', 'work', 'search_off', 'home', 'arrow_back',
  'dark_mode', 'light_mode', 'share', 'favorite', 'bookmark', 'verified', 'language',
  'chevron_left', 'chevron_right', 'search', 'content_copy', 'public', 'rss_feed',
  'alternate_email', 'calendar_today', 'schedule', 'rocket_launch', 'grid_view',
  'smartphone', 'dns', 'account_tree', 'bug_report', 'palette', 'description',
  'devices', 'cloud', 'check_circle', 'error', 'warning', 'info'
];
...
async function generateIconCSS(icons) {
  ...
  for (const materialIcon of icons) {
...
async function main() {
  ...
  console.log('Generating CSS for all project icons with currentColor support...\n');
  
  const css = await generateIconCSS(allIcons);
